...
class Sheet(tk.Frame):
    ...
    def change_theme(self, theme: str = "light blue", redraw: bool = True, re_binding_keys: bool = False) -> Sheet:
        if theme.lower() in ("light blue", "light_blue"):
            self.set_options(**theme_light_blue, redraw=False, re_binding_keys=re_binding_keys)
            self.config(bg=theme_light_blue["table_bg"])
        elif theme.lower() == "dark":
            self.set_options(**theme_dark, redraw=False, re_binding_keys=re_binding_keys)
            self.config(bg=theme_dark["table_bg"])
        elif theme.lower() in ("light green", "light_green"):
            self.set_options(**theme_light_green, redraw=False, re_binding_keys=re_binding_keys)
            self.config(bg=theme_light_green["table_bg"])
        elif theme.lower() in ("dark blue", "dark_blue"):
            self.set_options(**theme_dark_blue, redraw=False, re_binding_keys=re_binding_keys)
            self.config(bg=theme_dark_blue["table_bg"])
        elif theme.lower() in ("dark green", "dark_green"):
            self.set_options(**theme_dark_green, redraw=False, re_binding_keys=re_binding_keys)
            self.config(bg=theme_dark_green["table_bg"])
        elif theme.lower() == "black":
            self.set_options(**theme_black, redraw=False, re_binding_keys=re_binding_keys)
    ...
    def set_options(self, redraw: bool = True, re_binding_keys: bool = True, **kwargs) -> Sheet:
    ...
    # -----------------------------扩展-----------------------------
    def get_row_col_amount(self):
        return (len(self.MT.row_positions)-1, len(self.MT.col_positions)-1)

    def get_currently_selected_rc(self):
        r,c = None, None
        currentlySelected = self.get_currently_selected()  #当前选中行列
        try:
    ...
    def fix_select_row_col(self):
        r,c = None, None
        currentlySelected = self.get_currently_selected()  #当前选中行列
        try:
            r,c = currentlySelected.row, currentlySelected.column
            rAmount, cAmount = self.get_row_col_amount()
            if rAmount > 0 and cAmount > 0:
                r = min(max(r, 0), rAmount-1)
                c = min(max(c, 0), cAmount-1)
                self.select_cell(r, c, redraw=False)
        except Exception as e:
            pass

    def fix_select_col_with_tl_header(self, tl_header):
        r,c = None, None
        currentlySelected = self.get_currently_selected()  #当前选中行列
        try:
            r,c = currentlySelected.row, currentlySelected.column
            if c >= len(tl_header):
                self.select_cell(r, len(tl_header)-1, redraw=False)
        except Exception as e:
            pass

    def is_selected_out_of_range(self):
        r,c = None, None
        currentlySelected = self.get_currently_selected()  #当前选中行列
        try:
            r,c = currentlySelected.row, currentlySelected.column
            return self.is_rc_out_of_range(r,c)
        except Exception as e:
            return True
        return False

    def is_rc_out_of_range(self, r, c):
        if r == None or c == None:
            return True
        rAmount, cAmount = self.get_row_col_amount()
        if rAmount <= 0 or cAmount <= 0:
            return True
        if r < 0 or r > rAmount-1 or c < 0 or c > cAmount-1:
            return True
        return False

    def is_rc_completely_visible(self, r, c):
        return self.MT.cell_completely_visible(r=r, c=c)

    def is_rc_visible(self, r, c):
        return self.MT.cell_visible(r=r, c=c)

    def is_r_completely_visible(self, r):
        return self.MT.row_completely_visible(r=r)

    def is_r_visible(self, r):
        return self.MT.row_visible(r=r)

    def is_c_completely_visible(self, c):
        return self.MT.col_completely_visible(c=c)

    def is_c_visible(self, c):
        return self.MT.col_visible(c=c)

    def get_page_up_down_r(self, shift=1):
        height = self.MT.winfo_height()
        top = self.MT.canvasy(0)
        scrollto = top + height*shift
        if scrollto < 0:
            scrollto = 0
        rAmount, cAmount = self.get_row_col_amount()
        r = bisect_left(self.MT.row_positions, scrollto)
        rCurrent, _ = self.get_currently_selected_rc()
        if shift >= 0:
            r = r-1
            # if rCurrent and rCurrent == r:
            #     r += 1
            r = min(r, rAmount-1)
        else:
            # if rCurrent and rCurrent == r:
    ...
    def get_page_up_down_c(self, shift=1):
        height = self.MT.winfo_width()
        top = self.MT.canvasx(0)
        scrollto = top + height*shift
        if scrollto < 0:
            scrollto = 0
        rAmount, cAmount = self.get_row_col_amount()
        c = bisect_left(self.MT.col_positions, scrollto)
        rCurrent, _ = self.get_currently_selected_rc()
        if shift >= 0:
            c = c-1
            # if rCurrent and rCurrent == r:
    ...
    def set_col_width(
        ...
        return self.CH.set_col_width(
            col,
            width=width,
            only_set_if_too_small=only_set_if_too_small,
            displayed_only=displayed_only,
            recreate=recreate,
            return_new_width=return_new_width
        )

    def set_row_index_default_width(self, width=None, is_refresh=True):
        self.RI.set_default_width(width)
        if is_refresh:
            self.RI.reset_width()
