...
class MainTable(tk.Canvas):
    ...
    def row_completely_visible(self, r: int = 0) -> bool:
        c = 0
        cx1, cy1, cx2, cy2 = self.get_canvas_visible_area()
        x1, y1, x2, y2 = self.get_cell_coords(r, c)
        if cy1 <= y1 and cy2 >= y2:
            return True
        return False

    def row_visible(self, r: int = 0) -> bool:
        c = 0
        cx1, cy1, cx2, cy2 = self.get_canvas_visible_area()
        x1, y1, x2, y2 = self.get_cell_coords(r, c)
        if y1 <= cy2 or y2 >= cy1:
    ...
    def col_completely_visible(self, c: int = 0) -> bool:
        r = 0
        cx1, cy1, cx2, cy2 = self.get_canvas_visible_area()
        x1, y1, x2, y2 = self.get_cell_coords(r, c)
        if cx1 <= x1 and cx2 >= x2:
            return True
        return False

    def col_visible(self, c: int = 0) -> bool:
        r = 0
        cx1, cy1, cx2, cy2 = self.get_canvas_visible_area()
        x1, y1, x2, y2 = self.get_cell_coords(r, c)
        if x1 <= cx2 or x2 >= cx1:
