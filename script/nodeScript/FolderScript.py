...
def onEntryPathDrop(tmExUi, files=''):
    ...
    if not os.path.isdir(tlFile[0]):
        messagebox.showerror('错误','不是文件夹路径', parent=tmExUi['entryPath'])
        return
...
def getPath(data=None, tmExUi=None):
    ...
    if tmExUi:
        ...
        return data['folderPath'] if data != None and 'folderPath' in data else None